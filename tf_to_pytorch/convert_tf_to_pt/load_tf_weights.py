...
def load_efficientnet(model, checkpoint_file, model_name):
    ...
    if hasattr( model._blocks[0], '_se_reduce' ):
        conversion_table_for_first_block.update( {
    ...
    for i in range(len(model._blocks)):
        ...
        if is_first_block:
            ...
            if hasattr( model._blocks[i], '_se_reduce' ):
                conversion_table_block.update( {                
...
def load_and_save_temporary_tensorflow_model(model_name, model_ckpt, example_img= '../../example/img.jpg'):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        probs = eval_ckpt_driver.build_model(
            images,
            is_training=False,
        )
